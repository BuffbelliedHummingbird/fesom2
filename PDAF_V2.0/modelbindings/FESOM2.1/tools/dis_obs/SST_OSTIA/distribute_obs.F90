...
! Local variables
  ...
  INTEGER :: id_sst, id_my_sst, id_std, id_my_std
  ...
  REAL, ALLOCATABLE :: sst(:), my_sst(:), std(:), my_std(:)
  REAL :: stderr_sst
...
! ************************************************
  ...
  inpath = '/work/ollie/frbunsen/data/physics/SST/OSTIA/CORE2/'
  infile = 'OSTIA_SST_'//year_string//'0101_'//year_string//'1231_daily.nc'
  
  ! Path to and name stub of output files
  outpath = '/work/ollie/frbunsen/data/physics/SST/OSTIA/CORE2/dist_144/'
  outfile = 'OSTIA_SST_'//year_string//'0101_'//year_string//'1231_daily'//'_dist144'
...
! *******************************************
  ...
  s = s + 1
  stat(s) = NF_INQ_DIMID(ncid_in, 'T', id_dim)
  s = s + 1
...
! ************************************
  ...
  IF (twin_data) THEN
    s = 1
    stat(s) = NF_INQ_VARID(ncid_in, 'T', id_time)
    s = s + 1
  ...
  initoutfiles: DO pe = 0, npes - 1
     ...
     attstr  = 'daily SST observations from year '//year_string
     s = s + 1
...
! Read my_listXXXXX files
    ...
    ALLOCATE(my_std     (myDim_nod2D))
     ...
     stepsloop: DO iter = 1, steps
        ...
        IF (stat(1) /= NF_NOERR) &
             WRITE(*, *) 'NetCDF error in reading SST'
        ...
        IF (stat(1) /= NF_NOERR) &
             WRITE(*, *) 'NetCDF error in writing local SST'
     ...
     DEALLOCATE(myList_nod2D, myList_elem2D, my_sst, my_std)
  ...
  DEALLOCATE(cntsst, cntsstday, sst, std)
