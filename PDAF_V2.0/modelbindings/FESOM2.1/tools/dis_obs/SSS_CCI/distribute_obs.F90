...
! ************************************************
  ...
  inpath = '/albedo/work/projects/p_recompdaf/frbunsen/data/physics/SSS/CCI/CORE2/'//trim(year_string)//'/'
  infile = 'CCI-SSS-'//trim(year_string)//'-fullyear.nc'
  
  ! Path to and name stub of output files
  outpath = '/albedo/work/projects/p_recompdaf/frbunsen/data/physics/SSS/CCI/CORE2/dist_'//trim(npes_string)//'/'
  outfile = 'CCI_SSS_'//year_string//'_dist'//trim(npes_string)
...
!~     ALLOCATE(my_std     (myDim_nod2D))
     ...
     s = s + 1
     stat(s) = NF_INQ_VARID(ncid_in, 'sss', id_obs)
