...
! Local variables
  INTEGER :: i, s, iter, pe
  LOGICAL :: twin_data
  CHARACTER(len=300) :: inpath, outpath, distpath
  CHARACTER(len=300) :: infile, outfile, partfile, mylistfile
  CHARACTER(len=300) :: ncfile_in, ncfile_out
  CHARACTER(len=300) :: attstr
  CHARACTER(len=300) :: file_name                                       ! myListXXXXX numbered filenames
  CHARACTER(len=10)  :: mype_string
  INTEGER :: fileID                                                     ! ID for myListXXXXX files
  INTEGER :: ncid_in, ncid_out
  INTEGER :: id_dim, id_time, id_list
  INTEGER :: id_obs, id_my_obs
...
! ************************************************
  ...
  !infile  = 'chlobsdiff.runmean.smooth.core2.nc'
  !outfile = 'chlobsdiff.runmean.smooth.core2.dist'//trim(npes_string)
  
  infile  = 'chlobsdiff.runmean.core2.nc'
  outfile = 'chlobsdiff.runmean.core2.dist'//trim(npes_string)

  ! Path to and name of file holding global observations on model grid
  inpath = '/albedo/work/projects/p_recompdaf/frbunsen/modelruns/fesom2/update_BGC_tests/FREE/work/01/'
  
  ! Path to and name stub of output files
  outpath = trim(inpath)//'chlobsdiff_dist'//trim(npes_string)//'/'
...
! ************************************
  ...
  initoutfiles: DO pe = 0, npes - 1
     ...
     attstr  = 'Bias Total Surface Log-Chlorophyll 2010-2020 (FREE - OCCCI)'
     s = s + 1
     ...
     s = s + 1
     stat(s)=NF_DEF_VAR(ncid_out, 'bias', NF_DOUBLE, 2, dimids(1:2), id_my_obs)
     s = s + 1
...
! Read my_listXXXXX files
    ...
    ALLOCATE(my_obs     (myDim_nod2D))
     ...
     s = s + 1
     stat(s) = NF_INQ_VARID(ncid_in, 'obsdiff', id_obs)

     ! Get ID for local obs field
     s = s + 1
     stat(s) = NF_INQ_VARID(ncid_out, 'obsdiff', id_my_obs)
     ...
     stepsloop: DO iter = 1, steps
        ...
        DO i = 1, nod2d_part(pe+1)
           ...
           IF (my_obs(i)>-999.0 .AND. my_obs(i)<999.0) cntobs(iter) = cntobs(iter)+1
           IF (iter==1) THEN
              IF (my_obs(i)>-999.0 .AND. my_obs(i)<999.0) cntobsday(pe+1) = cntobsday(pe+1)+1
           END IF
        END DO
     ...
     DEALLOCATE(myList_nod2D, myList_elem2D, my_obs)
  ...
  DEALLOCATE(cntobs, cntobsday, obs)
