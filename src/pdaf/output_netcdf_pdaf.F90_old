...
! ----- DEFINE DIMENSIONS ------------------------------
       ...
       s = s + 1
!~        stat(s) = NF_DEF_DIM(fileid, 'elem',       mesh_fesom%elem2D, DimId_elem)             
!~        s = s + 1
...
! ----- Smoother F I E L D S 
       ...
       smootherA: IF (dim_lag > 0) THEN
          ...
          dimarray(2) = DimId_nz1
          dimarray(3) = DimId_iter
          stat(s) = NF_DEF_VAR(fileid, 'temp_s', nf_prec, 3, dimarray(1:3), VarId_temps)
          s = s + 1
          stat(s) = NF_DEF_VAR(fileid, 'salt_s', nf_prec, 3, dimarray(1:3), VarId_salts)
          s = s + 1
         ...
         stat(s) = NF_DEF_VAR(fileid, 'u_s', nf_prec, 2, dimarray(1:3), VarId_us)
         s = s + 1
         stat(s) = NF_DEF_VAR(fileid, 'v_s', nf_prec, 2, dimarray(1:3), VarId_vs)
         ...
         stat(s) = NF_DEF_VAR(fileid, 'w_s', nf_prec, 2, dimarray(1:3), VarId_ws)
         s = s + 1
...
! ----- Effective observation dimensions and localization radii
       ...
       s = s + 1
       stat(s) = NF_DEF_VAR(fileid, 'rms_w_ini', nf_prec, 1, dim1, VarId_rmswini) 
       s = s + 1
       ...
       smootherB: IF (dim_lag > 0) THEN
          ...
          s = s + 1
          stat(s) = NF_DEF_VAR(fileid, 'rms_w_s', nf_prec, 1, DimId_iter, VarId_rmsws)
          s = s + 1
...
!~        stat(s) = NF_PUT_ATT_TEXT(fileid, VarId_a,         'long_name',   21, 'sea-ice concentration')
       ...
       stat(s) = NF_PUT_ATT_TEXT(fileid, VarId_sshi,      'long_name',   24, 'zonal velocity - initial') 
       s = s + 1
       stat(s) = NF_PUT_ATT_TEXT(fileid, VarId_sshi,      'units',       16, 'meter per second') 
       s = s + 1
       ...
       stat(s) = NF_PUT_ATT_TEXT(fileid, VarId_sshi,      'long_name',   29, 'meridional velocity - initial') 
       s = s + 1
       stat(s) = NF_PUT_ATT_TEXT(fileid, VarId_sshi,      'units',       16, 'meter per second') 
       s = s + 1
       ...
       stat(s) = NF_PUT_ATT_TEXT(fileid, VarId_sshi,      'long_name',   27, 'vertical velocity - initial') 
       s = s + 1
       stat(s) = NF_PUT_ATT_TEXT(fileid, VarId_sshi,      'units',       16, 'meter per second') 
       s = s + 1
...
! !INTERFACE: 
  SUBROUTINE write_netcdf_pdaf(writetype, write_pos_da, iteration, dim, state_l, &
   ...
   USE mod_assim_pdaf, ONLY: write_3D_monthly_mean
! ! USES:
    IMPLICIT NONE
...
ALLOCATE(uv_temp_g (mesh_fesom%nl-1, mesh_fesom%nod2D)) ! mesh_fesom%elem2D))
...
! ----- INQUIRE VARIABLE IDs
  ...
  IF (writetype=='i') THEN
     ...
     s = s + 1
     stat(s) = NF_INQ_VARID(fileid, "w_ini",      VarId_w) 
     s = s + 1
     ...
     s = s + 1
     stat(s) = NF_INQ_VARID(fileid, "rms_w_ini",       VarId_rmsw) 
     s = s + 1
...
!~      s = s + 1
  ...
  ELSE IF (writetype=='s') THEN
     ...
     s = s + 1
     stat(s) = NF_INQ_VARID(fileid, "w_s",      VarId_w) 
     s = s + 1
     ...
     s = s + 1
     stat(s) = NF_INQ_VARID(fileid, "rms_w_s",       VarId_rmsw) 
     s = s + 1
...
!~   write(*,*) 'fileid: ', fileid
!~   write(*,*) 'VarId_asmlstep', VarId_asmlstep
!~   write(*,*) 'VarId_iter', VarId_iter
!~   write(*,*) 'write_pos_da', write_pos_da
!~   write(*,*) 'istep_asml', istep_asml
...
uv: DO n = 2,3
	...
	! CALL gather_elem_pdaf(my_uv_temp, uv_temp_g)
...
! Print screen information
    IF (writepe) THEN
       IF (writetype == 'i') THEN
          WRITE (*, '(a, 8x, a, i9, a, i5)') 'FESOM-PDAF', 'Write initial ocean state ENS to NetCDF at step ', &
               istep, ' position ', write_pos_da
       ELSE IF (writetype== 'f') THEN
       ...
       ELSE IF (writetype== 'a') THEN
          WRITE (*, '(a, 8x, a, i9, a, i5)') 'FESOM-PDAF', 'Write ocean analysis ENS to NetCDF at step ', &
               istep, ' position ', write_pos_da
       ELSE IF (writetype== 's') THEN
          WRITE (*, '(a, 8x, a, i9, a, i5)') 'FESOM-PDAF', 'Write smoothed ocean ENS to NetCDF at step ', &
               istep, ' position ', write_pos_da
       END IF
    END IF
