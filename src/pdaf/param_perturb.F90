...
subroutine do_perturb_param_bio()
        ...
        ! alfa
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: alfa            ', alfa, ' (specified)'
        iseed(1)=1
        iseed(2)=3
        iseed(3)=31+task_id ! different seed for each ensemble member
        iseed(4)=37
        CALL perturb_lognormal(alfa, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: alfa            ', alfa, ' on task ', task_id
        
        ! alfa_d
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: alfa_d          ', alfa_d, ' (specified)'
        iseed(1)=2
        iseed(2)=5
        iseed(3)=29+task_id
        iseed(4)=41
        CALL perturb_lognormal(alfa_d, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: alfa_d          ', alfa_d, ' on task ', task_id
       
        ! P_cm
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: P_cm            ', P_cm, ' (specified)'
        iseed(1)=2
        iseed(2)=4
        iseed(3)=23 + task_id
        iseed(4)=43
        CALL perturb_lognormal(P_cm, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: P_cm            ', P_cm, ' on task ', task_id
        
        ! P_cm_d
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: P_cm_d          ', P_cm_d, ' (specified)'
        iseed(1)=7
        iseed(2)=4
        iseed(3)=19 + task_id
        iseed(4)=47
        CALL perturb_lognormal(P_cm_d, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: P_cm_d          ', P_cm_d, ' on task ', task_id
        
        ! Chl2N_max
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: Chl2N_max       ', Chl2N_max, ' (specified)'
        iseed(1)=14
        iseed(2)=15
        iseed(3)=17 + task_id
        iseed(4)=53
        CALL perturb_lognormal(Chl2N_max, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: Chl2N_max       ', Chl2N_max, ' on task ', task_id
        
        ! Chl2N_max_d
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: Chl2N_max_d     ', Chl2N_max_d, ' (specified)'
        iseed(1)=15
        iseed(2)=16
        iseed(3)=13 + task_id
        iseed(4)=59
        ...
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: deg_Chl         ', deg_Chl, ' on task ', task_id
        
        ! deg_Chl_d
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: deg_Chl_d       ', deg_Chl_d, ' (specified)'
        iseed(1)=104
        iseed(2)=97
        iseed(3)=7 + task_id
        iseed(4)=67
        CALL perturb_lognormal(deg_Chl_d, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: deg_Chl_d       ', deg_Chl_d, ' on task ', task_id
        
        ! graz_max
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: graz_max        ', graz_max, ' (specified)'
        iseed(1)=33
        iseed(2)=16
        iseed(3)=3 + task_id
        iseed(4)=73
        CALL perturb_lognormal(graz_max, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: graz_max        ', graz_max, ' on task ', task_id
        
        ! graz_max2
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: graz_max2       ', graz_max2, ' (specified)'
        iseed(1)=3
        iseed(2)=6
        iseed(3)=5 + task_id
        iseed(4)=71
        CALL perturb_lognormal(graz_max2, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: graz_max2       ', graz_max2, ' on task ', task_id

        ! grazEff
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: grazEff         ', grazEff, ' (specified)'
        iseed(1)=2
        iseed(2)=1
        iseed(3)=6+3*task_id
        iseed(4)=13
        CALL perturb_lognormal(grazEff, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: grazEff         ', grazEff, ' on task ', task_id
        
        ! grazEff2
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: grazEff2        ', grazEff2, ' (specified)'
        iseed(1)=6
        iseed(2)=3
        iseed(3)=97+101*task_id
        iseed(4)=11
        CALL perturb_lognormal(grazEff2, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: grazEff2        ', grazEff2, ' on task ', task_id
        
        ! VDet
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: VDet            ', VDet, ' (specified)'
        iseed(1)=68
        iseed(2)=54
        iseed(3)=63+3*task_id
        iseed(4)=21
        CALL perturb_lognormal(VDet, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: VDet            ', VDet, ' on task ', task_id
        
        ! VDet_zoo2
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: vdet_zoo2       ', vdet_zoo2, ' (specified)'
        iseed(1)=87
        iseed(2)=28
        iseed(3)=93+5*task_id
        iseed(4)=23
        CALL perturb_lognormal(vdet_zoo2, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: vdet_zoo2       ', vdet_zoo2, ' on task ', task_id
        
        ! Vdet_a
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: vdet_a          ', vdet_a, ' (specified)'
        iseed(1)=45
        iseed(2)=65
        iseed(3)=41+7*task_id
        iseed(4)=27
        CALL perturb_lognormal(vdet_a, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: vdet_a          ', vdet_a, ' on task ', task_id
        
        ! k_din
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: k_din           ', k_din, ' (specified)'
        iseed(1)=26
        iseed(2)=87
        iseed(3)=56+4*task_id
        iseed(4)=29
        CALL perturb_lognormal(k_din, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: k_din           ', k_din, ' on task ', task_id
        
        ! k_din_d
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: k_din_d         ', k_din_d, ' (specified)'
        iseed(1)=61
        iseed(2)=15
        ...
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: res_phy         ', res_phy, ' on task ', task_id
        
        ! res_phy_d
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: res_phy_d       ', res_phy_d, ' (specified)'
        iseed(1)=20
        iseed(2)=50
        iseed(3)=30+9*task_id
        iseed(4)=37
        CALL perturb_lognormal(res_phy_d, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: res_phy_d       ', res_phy_d, ' on task ', task_id
        
        ! rho_N
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: rho_N           ', rho_N, ' (specified)'
        iseed(1)=12
        iseed(2)=24
        iseed(3)=36+11*task_id
        iseed(4)=45
        CALL perturb_lognormal(rho_N, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: rho_N           ', rho_N, ' on task ', task_id
        
        ! rho_C1
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: rho_c1          ', rho_c1, ' (specified)'
        iseed(1)=9
        iseed(2)=38
        iseed(3)=44+3*task_id
        iseed(4)=47
        CALL perturb_lognormal(rho_c1, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: rho_c1          ', rho_c1, ' on task ', task_id
        
        ! lossN
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: lossN           ', lossn, ' (specified)'
        iseed(1)=18
        iseed(2)=51
        iseed(3)=54+8*task_id
        iseed(4)=53
        CALL perturb_lognormal(lossn, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: lossN           ', lossn, ' on task ', task_id
        
        ! lossN_d
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: lossN_d         ', lossN_d, ' (specified)'
        iseed(1)=48
        iseed(2)=73
        iseed(3)=35+5*task_id
        iseed(4)=55
        CALL perturb_lognormal(lossN_d, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: lossN_d         ', lossN_d, ' on task ', task_id
        
        ! lossC
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: lossC           ', lossC, ' (specified)'
        iseed(1)=94
        iseed(2)=2
        iseed(3)=22+12*task_id
        iseed(4)=63
        CALL perturb_lognormal(lossC, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: lossC           ', lossc, ' on task ', task_id
        
        ! lossC_d
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: lossC_d         ', lossC_d, ' (specified)'
        iseed(1)=41
        iseed(2)=74
        iseed(3)=31+8*task_id
        iseed(4)=67
        CALL perturb_lognormal(lossC_d, perturb_scale, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: lossC_d         ', lossc_d, ' on task ', task_id
        
        ! reminN
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: reminN          ', reminn, ' (specified)'
        iseed(1)=80
        iseed(2)=25
        iseed(3)=97+101*task_id
        iseed(4)=69
        CALL perturb_lognormal(reminn, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: reminN          ', reminn, ' on task ', task_id
        
        ! reminC    
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: reminC          ', reminC, ' (specified)'
        iseed(1)=13
        iseed(2)=15
        iseed(3)=66+17*task_id
        iseed(4)=71
        CALL perturb_lognormal(reminC, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: reminC          ', reminc, ' on task ', task_id
        
        ! calc_prod_ratio   
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: calc_prod_ratio ', calc_prod_ratio, ' (specified)'
        iseed(1)=11
        iseed(2)=12
        iseed(3)=55+5*task_id
        iseed(4)=73
        CALL perturb_lognormal(calc_prod_ratio, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: calc_prod_ratio ', calc_prod_ratio, ' on task ', task_id
        
        ! res_het
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: res_het         ', res_het, ' (specified)'
        iseed(1)=11
        iseed(2)=12
        iseed(3)=55+5*task_id
        iseed(4)=73
        CALL perturb_lognormal(res_het, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: res_het         ', res_het, ' on task ', task_id
        
        ! res_zoo2
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: res_zoo2        ', res_zoo2, ' (specified)'
        iseed(1)=11
        iseed(2)=12
        iseed(3)=55+5*task_id
        iseed(4)=73
        CALL perturb_lognormal(res_zoo2, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: res_zoo2        ', res_zoo2, ' on task ', task_id
        
        ! biosynth
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: biosynth        ', biosynth, ' (specified)'
        iseed(1)=11
        iseed(2)=12
        iseed(3)=55+5*task_id
        iseed(4)=73
        CALL perturb_lognormal(biosynth, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: biosynth        ', biosynth, ' on task ', task_id
        
        ! calc_diss_rate
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: calc_diss_rate  ', calc_diss_rate, ' (specified)'
        iseed(1)=11
        iseed(2)=12
        iseed(3)=55+5*task_id
        iseed(4)=73
        CALL perturb_lognormal(calc_diss_rate, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: calc_diss_rate  ', calc_diss_rate, ' on task ', task_id
        
        ! calc_diss_rate2
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: calc_diss_rate2 ', calc_diss_rate2, ' (specified)'
        iseed(1)=11
        iseed(2)=12
        iseed(3)=55+5*task_id
        iseed(4)=73
        CALL perturb_lognormal(calc_diss_rate2, perturb_scaleD, iseed)
        IF (mype_model==0) WRITE(*,'(a31,g0,a9,i3)') 'param_perturb: calc_diss_rate2 ', calc_diss_rate2, ' on task ', task_id
...
subroutine do_perturb_param_phy()
       ...
       USE o_param, &
        ...
        ! K_ver
        IF (mype_model==0 .and. task_id==1) WRITE(*,'(a31,g0,a15)') 'param_perturb: K_ver           ', K_ver, ' (specified)'
        iseed(1)=11
        iseed(2)=12
        iseed(3)=55+5*task_id
        iseed(4)=73
