...
MODULE obs_o2_merged_pdafomi
  ...
  LOGICAL :: o2_merged_errorfiles = .False.  ! True:  read observation error from observation data
                                             ! False: use constant observation error
...
! **********************************
    ...
    ELSE ! (i.e. dim_obs_p > 0)
      ...
      if (o2_merged_errorfiles) then
         ! Reading observation error
         ncstat = nf90_inq_varid(ncid,'ERR', id_err)
         if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_o2_merged_pdafomi - Error getting id_err from netCDF'
         ncstat = nf90_get_var(ncid, id_err, std_obs_p_reps)
         if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_o2_merged_pdafomi - Error reading err from netCDF'
      else
         ! Use constant observation error
         std_obs_p_reps(:) = rms_obs_o2_merged
      endif
      ...
      do_elem_p_reps: DO e_reps=1, dim_obs_p_reps
         ...
         ! else, continue here:
            ...
            ! --> already have observations of elem(e_reps) at e_found:
                ...
                if (o2_merged_errorfiles) std_obs_p_sorted(e_found) = std_obs_p_sorted(e_found) + std_obs_p_reps(e_reps)
            ...
            ! --> is the first or only observation at elem(e_reps)
               ...
               if (o2_merged_errorfiles) std_obs_p_sorted(dim_obs_p) = std_obs_p_reps(e_reps)
      ...
      DO e=1,dim_obs_p
        ...
        if (o2_merged_errorfiles) std_obs_p(e) = std_obs_p_sorted(e)/REAL(numrep_p(e))
      ...
      if (.not. o2_merged_errorfiles) std_obs_p(:) = rms_obs_o2_merged
      ...
      IF (o2_merged_errorfiles) THEN
         ...
         ENDDO
      ELSE
         ivariance_obs_p(:) = 1.0 / (rms_obs_o2_merged * rms_obs_o2_merged)
      ENDIF
      if (isPP) then
    ...
    IF (mype_filter == 0) &
            ...
            '--- O2 merged observations excluded due to topography: ', cnt_ex_dry
    ...
    IF (mype_filter == 0) &
        ...
        '--- Full merged O2 observations have been gathered; dim_obs is ', dim_obs
    ...
    IF (thisobs%doassim == 1) THEN
       ...
       if (n_sweeps>1) then
          ...
          elseif (domain_p==myDim_nod2D+1) then
             if (mype_filter==0) &
                  write (*,'(a,4x,a)') 'FESOM-PDAF', &
                  '--- BIO sweep: set ivar_obs_f for merged O2 to normal'
