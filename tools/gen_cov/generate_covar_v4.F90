...
! ******************************************************************
  ...
  ! Topography on CORE2 mesh: maximum number of 46 wet cells
  nlmax = 46
  ...
  WRITE (*,'(10x,1x,a25,i12)') 'nlmax      ', nlmax
...
! ******************************************************************
  ...
  do b=fmin, fmax
     if (fproperties(b) % ndims == 2) dim_fields(b) = nod2*nlmax
...
! ****************************
  ...
  ALLOCATE(u_orig(nlmax*elem))
  ALLOCATE(v_orig(nlmax*elem))
  ...
  ! ****************************
  startv3(1) = 1
  countv3(1) = nlmax ! get nlmax of nz elements starting from 1
  startv3(2) = 1     
  ...
  DO n = 1,nod2
     DO l = 1, nlevels_nod2D(n)-1
        tvol=0.0
        tx  =0.0
        ty  =0.0
           DO c = 1, nod_in_elem2D_num(n)
               el = nod_in_elem2D(c,n)
               if ((nlevels(el)-1)<l) cycle
               tvol = tvol + elem_area(el)
               tx = tx + u_orig((el-1)*nlmax+l) *elem_area(el)
               ty = ty + v_orig((el-1)*nlmax+l) *elem_area(el)
           END DO
        states(offsets(idx% u)+(n-1)*nlmax+l,iter/ostep+1) = tx/tvol ! u
        states(offsets(idx% v)+(n-1)*nlmax+l,iter/ostep+1) = ty/tvol ! v
     END DO
  END DO
...
! ****************************
  ...
  ! 3D-fields
  startv3(1) = 1     
  countv3(1) = nlmax ! get nlmax of nz values starting from 1
  startv3(2) = 1
