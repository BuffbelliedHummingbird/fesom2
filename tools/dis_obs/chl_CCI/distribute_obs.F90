...
! mesh. The observation information is read from a NetCDF file 
! holding the global observation information for one year.

! The year needs to be passed as command line argument.
...
! Local variables
  ...
  INTEGER :: id_dim, id_std, id_time, id_list
  INTEGER :: id_obs, id_my_obs, id_my_std
...
! ************************************************
  ...
  inpath = '/albedo/work/projects/p_recompdaf/frbunsen/data/BGC/chlsurf_OC-CCI/CORE2_daily/'
  infile = 'CCI-OC-'//trim(year_string)//'-fullyear.nc'
  
  ! Path to and name stub of output files
  outpath = '/albedo/work/projects/p_recompdaf/frbunsen/data/BGC/chlsurf_OC-CCI/CORE2_daily/dist_'//trim(npes_string)//'/'
  outfile = 'CCI_OC_'//year_string//'_dist'//trim(npes_string)
...
! Read my_listXXXXX files
    ...
    ALLOCATE(my_std     (myDim_nod2D))
     ...
     s = s + 1
     stat(s) = NF_INQ_VARID(ncid_in, 'chlor_a', id_obs)
     s = s + 1
     stat(s) = NF_INQ_VARID(ncid_in, 'chlor_a_log10_rmsd', id_std)
     ...
     stepsloop: DO iter = 1, steps
        ...
        stat(2) = NF_GET_VARA_DOUBLE(ncid_in, id_std, startv, countv, std)
        IF (stat(2) /= NF_NOERR) &
        ...
        stat(2) = NF_PUT_VARA_DOUBLE(ncid_out, id_my_std, startv, countv, my_std)
        IF (stat(1) /= NF_NOERR) &
